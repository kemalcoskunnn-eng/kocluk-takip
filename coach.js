...
async function load(){
  ...
  for(const p of profiles||[]){
    ...
    chartRows.push({name:p.full_name,tyt:tNet,ayt:aNet});
  ...
  coachChart=new Chart(document.getElementById("coachChart"),{
    type:"bar",
    data:{labels:chartRows.map(x=>x.name),datasets:[
      {label:"TYT",data:chartRows.map(x=>x.tyt),backgroundColor:"rgba(37,99,235,.78)",borderRadius:7,maxBarThickness:34},
      {label:"AYT",data:chartRows.map(x=>x.ayt),backgroundColor:"rgba(217,119,6,.78)",borderRadius:7,maxBarThickness:34}
    ]},
    options:{responsive:true,maintainAspectRatio:false,animation:false,normalized:true,plugins:{legend:{labels:{usePointStyle:true,boxWidth:7,font:{size:11}}}},scales:{y:{beginAtZero:true,max:120,grid:{color:"rgba(148,163,184,.13)"},ticks:{font:{size:10}}},x:{grid:{display:false},ticks:{font:{size:11}}}}}
  });
...
(async()=>{
  ...
  supabaseClient.channel("coach-exams-live")
    .on("postgres_changes",{event:"*",schema:"public",table:"exams"},()=>{window.clearTimeout(window.__refreshTimer);window.__refreshTimer=setTimeout(load,250);})
    .subscribe();
})();
